class Message
  ...
  def self.create_env_datum(entry, thing_id, thing_name, temp_c_field, temp_f_field, humidity_field, light_field)
    e = EnvDatum.new
    e.thing_id = thing_id
    e.temp_c = entry[temp_c_field]
    e.temp_f = entry[temp_f_field]
    e.humidity = entry[humidity_field]
    e.light = entry[light_field]
  ...
  def valid?
    return false unless \
      channel["field1"] == "Temp C 1" && \
      channel["field2"] == "Temp C 2" && \
      channel["field3"] == "Temp F 1" && \
      channel["field4"] == "Temp F 2" && \
      channel["field5"] == "Humidity 1" && \
      channel["field6"] == "Humidity 2" && \
      channel["field7"] == "Light 1" && \
      channel["field8"] == "Light 2"
  ...
  def data
    feeds.map do |datum|
      if datum["field1"].blank? and datum["field3"].blank? and datum["field5"].blank? and datum["field7"].blank?
        idem_add_data_thing2(datum)
      elsif datum["field2"].blank? and datum["field4"].blank? and datum["field6"].blank? and datum["field8"].blank?
  ...
  def idem_add_data_thing1(datum)
    ...
    unless e
      ...
      thing_id = 1
      Message.create_env_datum(datum, thing_id, "thing1", "field1", "field3", "field5", "field7")
  ...
  def idem_add_data_thing2(datum)
    ...
    unless e
      ...
      thing_id = 2
      Message.create_env_datum(datum, thing_id, "thing2", "field2", "field4", "field6", "field8")
      Rails.logger.debug "saved"
    end
    return e
  end
